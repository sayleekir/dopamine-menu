...
const VIEWERS_API = "https://dopamine-menu-api.karekar-mohit.workers.dev/viewers";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const viewersEl = timerEl.querySelector(".timer-viewers");
  const baseCategory = CATEGORY_INDEX[timerEl.dataset.category];
  const subCategory = 0;
  let viewerPollId = null;
  let hasJoined = false;
  ...
  function sendLeave() {
    if (baseCategory === undefined || !hasJoined) return;
    hasJoined = false;
    const payload = JSON.stringify({ categoryList: [baseCategory, subCategory] });
    const url = `${VIEWERS_API}/leave`;
  ...
  function fetchViewerCount() {
    if (baseCategory === undefined || !viewersEl) return;
    fetch(`${VIEWERS_API}?categoryId=${baseCategory}_${subCategory}`)
      .then((res) => res.text())
      .then((text) => {
        const count = parseInt(text, 10);
        if (!Number.isNaN(count)) {
          viewersEl.textContent = `${count} ${count === 1 ? "person" : "people"} doing this now`;
        }
      })
      .catch(() => {});
  }
  ...
  function startViewerTracking() {
    if (viewerPollId || baseCategory === undefined) return;
    postViewer();
    fetchViewerCount();
    viewerPollId = setInterval(fetchViewerCount, VIEWERS_POLL_MS);
  }
